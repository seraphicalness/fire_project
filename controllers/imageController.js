...
export const uploadProfileImage = (req, res) => {
    upload.single('image')(req, res, (err) => {
      if (err) {
        ...
        return res.status(500).json({ message: "프로필 이미지 업로드 중 오류 발생" });
      }
...
export const uploadBackgroundImage = (req, res) => {
    upload.single('image')(req, res, (err) => {
      if (err) {
        ...
        return res.status(500).json({ message: "배경 이미지 업로드 중 오류 발생" });
      }
      const imageUrl = `${req.protocol}://${req.get("host")}/uploads/background/${req.file.filename}`;
      res.json({ imageUrl });
    });
  };
